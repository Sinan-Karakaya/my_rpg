...
const unsigned int perm[] =
    ...
    196, 135, 130, 130, 188, 159, 86, 164, 100, 109, 198, 173, 186, 3, 64, 52,
...
static void condit(float x, float y, perlin_t *perlin)
{
    ...
    perlin->gi0 = perm[((int)(x) + perm[(int)(y) % 254]) % 254] % 8;
    perlin->gi1 = perm[((int)(x) + 1 + perm[(int)(y) % 254]) % 254] % 8;
    perlin->gi2 = perm[((int)(x) + perm[((int)(y) + 1) % 254]) % 254] % 8;
    perlin->gi3 = perm[((int)(x) + 1 + perm[((int)(y) + 1) % 254]) % 254] % 8;
...
float perlin(float x, float y, float res)
{
    ...
    perlin->tmp * perlin->tmp;
    to_return = perlin->Li1 + perlin->Cy * (perlin->Li2 - perlin->Li1);
    free(perlin);
    return to_return;
}