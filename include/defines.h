...
#define CALCUL_1(x, y, i, j) ((point[1].x - point[0].x) / 150 * \
(offset_tab[rpg->world->object_map[i][j]].y - \
offset_tab[rpg->world->object_map[i][j]].x)), \
-(point[1].x - point[0].x) * 0.1

#define CALCUL_2(x, y, i, j) point[0].x + ((point[1].x - \
point[0].x) / 150 * offset_tab[rpg->world->object_map[i][j]].x), point[0].y
